...
ADM2_path = paste(path, "data/firm-data/Indonesia/Shapefile/IDN_adm_shp/IDN_adm2.shp", sep="")
ADM0_path = paste(path, "data/firm-data/Indonesia/Shapefile/IDN_adm_shp/IDN_adm0.shp", sep="")
...
grid_vect_path = paste(path, "data/tropical-cyclones/intermediate/Indonesia/grid_0p1deg.shp", sep="")

#path to a 0.1x0.1deg grid for a country and its 200km buffer (created in QGIS): raster
grid_rast_path = paste(path, "./data/tropical-cyclones/intermediate/Indonesia/grid_0p1deg.tif", sep="")

#path to country-specific hurricane data: shape file of hurricanes' eyes in 200km buffer (created in QGIS)
hurr_path = paste(path, "data/tropical-cyclones/intermediate/Indonesia/subset_1978_2015_200km.shp", sep="")
...
hurr$time_diff <- NA
hurr[2:nrow(hurr), ]$time_diff <- as.numeric(seconds(diff(hurr$ISO)/3600 - 1))
...
#collapsed <- panel[panel$year==1980,c("site_latitude","site_longitude","storm")]
