...
#bboxVietnam <- st_bbox(vietnamADM0, crs = st_crs(linda_east))
#gridVietnam <- st_sf(st_make_grid(bboxVietnam, cellsize=c(0.1, 0.1), crs = st_crs(linda_east)))

#gridVietnam <- st_read("data/firm-data/Vietnam/gadm36_VNM_shp/grid_bbox.shp")
#gridVietnam <- st_transform(gridVietnam, crs = st_crs(linda_east))

gridVietnam <- st_read("data/firm-data/Vietnam/gadm36_VNM_shp/grid_bbox_buffer.shp")
gridVietnam <- st_transform(gridVietnam, crs = st_crs(linda_east))
...
onePointHurricaneClean$bearing_eye_site <- bearing(site, eye)
onePointHurricaneClean$bearing_eye_site[onePointHurricaneClean$site_longitude > onePointHurricaneClean$USA_LON] <- +
           360 + onePointHurricaneClean$bearing_eye_site[onePointHurricaneClean$site_longitude > onePointHurricaneClean$USA_LON]
onePointHurricaneClean$T <- onePointHurricaneClean$bearing_eye_site - onePointHurricaneClean$bearing_eye_next +180

#5.1 Other way?


# 6. Calculate the Speeds at each point
onePointHurricaneClean$V_s <- with(onePointHurricaneClean, F*(V_m - S* (1-sin(T* pi/180))*V_h*0.5)* +
  ((R_m/R)^B*exp(1-(R_m/R)^B))^(0.5))*0.514444

end_time <- Sys.time()
end_time - start_time

onePointHurricaneClean$V_s_m_s <- onePointHurricaneClean$V_s*0.514444

ggplot(data = onePointHurricaneClean) +
  geom_sf(aes(fill = V_s_m_s), colour = NA) +
  scale_fill_gradient(low = 'white', high = 'red', name = "Sustained Wind Speed (m/s)") +
  geom_sf(data =vietnamADM2['GID_0'] , fill = NA) +
